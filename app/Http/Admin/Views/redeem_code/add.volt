...
{% block content %}
    ...
    <form class="layui-form kg-form" method="POST" action="{{ url({'for':'admin.redeem_code.create'}) }}">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                {% for value in [1,5,10,30,50,100] %}
                    {% set checked = value == 1 ? 'checked="checked"' : '' %}
                    <input type="radio" name="insert_count" value="{{ value }}" title="{{ value }}个" {{ checked }}>
                {% endfor %}
            </div>
        </div>
        <div class="layui-form-item">
